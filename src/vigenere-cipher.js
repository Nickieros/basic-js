...
class VigenereCipheringMachine {
  ...
  constructor(encrypt) {
    for(let j = 0; j < 26; j++){
      for(let i = String.fromCharCode('A'.charCodeAt(0) + j); this.table[j].length < 26; i = String.fromCharCode(i.charCodeAt(0) + 1)){
        this.table[j].push(i);
        if('Z' === i) i = '@';
      }
      this.table.push([]);
  ...
  encrypt(message, key) {
    if(undefined === message  || undefined === key) throw new Error
  ...
  decrypt(encryptedMessage, key) {
    if(undefined === encryptedMessage  || undefined === key) throw new Error
    let result = '';
    let n = 0;
    key = key.toUpperCase();
